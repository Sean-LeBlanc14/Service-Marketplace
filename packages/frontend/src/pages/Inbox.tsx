    ...
    <div className="offer-dialog-backdrop">
      <div className="offer-dialog">
        ...
        <p className="offer-dialog-hint">
          Enter the amount you would like to offer for this
          service.
        </p>
        <input
          type="number"
        ...
        <div className="offer-dialog-actions">
          <button
            className="offer-dialog-send"
            onClick={handleSend}>
            Send Offer
          </button>
          <button
            className="offer-dialog-cancel"
            onClick={onCancel}>
            Cancel
          </button>
        </div>
...
function MessageBubble({
  ...
  async function respond(accept: boolean) {
    ...
    try {
      const endpoint = accept
        ? API_ENDPOINTS.conversations.acceptOffer(
            conversationId,
            message.id
          )
        : API_ENDPOINTS.conversations.rejectOffer(
            conversationId,
            message.id
          );
  ...
  if (message.type === "PRICE_OFFER") {
    return (
      <div
        className={`message-bubble offer-bubble ${isMine ? "mine" : "theirs"}`}>
        <span className="message-sender">
          {isMine ? "You" : message.senderName}
        </span>
        <div className="offer-amount">
          ${Number(message.offeredPrice).toFixed(2)}
        </div>
  ...
  if (message.type === "PRICE_ACCEPTED") {
    return (
      <div
        className={`message-bubble accepted-bubble ${isMine ? "mine" : "theirs"}`}>
        <span className="message-sender">
          {isMine ? "You" : message.senderName}
        </span>
        <p className="offer-response-text">{message.content}</p>
        <div className="offer-not-final-notice">
          This is not a formal agreement. To finalize, the
          customer must create a booking and the provider must
          confirm via email.
        </div>
        <span className="message-time">
          {formatTime(message.createdAt)}
  ...
  if (message.type === "PRICE_REJECTED") {
    return (
      <div
        className={`message-bubble rejected-bubble ${isMine ? "mine" : "theirs"}`}>
        <span className="message-sender">
          {isMine ? "You" : message.senderName}
        </span>
        <p className="offer-response-text">{message.content}</p>
...
export default function Inbox() {
  ...
  const { latestMessage, refreshUnreadCounts } =
    useWebSocketContext();
  ...
  const isProvider =
    selectedConversation?.providerId === currentUserId;
  const isCustomer =
    selectedConversation?.customerId === currentUserId;

  const loadConversations = useCallback(async () => {
    try {
      const res = await fetch(API_ENDPOINTS.conversations.all, {
        headers: authHeaders()
      });
      if (res.ok)
        setConversations(
          (await res.json()) as ApiConversation[]
        );
    } catch {
      // silently ignore
    }
  ...
  async function sendText() {
    ...
    try {
      const res = await fetch(
        API_ENDPOINTS.conversations.sendMessage(selectedId),
        {
          method: "POST",
          headers: authHeaders(),
          body: JSON.stringify({
          ...
          return (
            <button
              ...
              <div className="inbox-conv-sub">
                <span className="inbox-conv-service">
                  {c.serviceTitle}
                </span>
                {c.unreadCount > 0 && (
        ...
        {selectedConversation && (
          <>
            <header className="inbox-chat-header">
              <div>
                <p className="inbox-chat-service">
